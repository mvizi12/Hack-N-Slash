...
void UResetAttackN::Notify(USkeletalMeshComponent *MeshComp, UAnimSequenceBase *Animation, const FAnimNotifyEventReference &EventReference)
{
    UCombatComponent* combatComp {MeshComp->GetOwner()->FindComponentByClass<UCombatComponent>()};
	if (IsValid(combatComp)) {combatComp->HandleResetAttack();}

    UCombatEnemyComponent* combatEnemyComp {MeshComp->GetOwner()->FindComponentByClass<UCombatEnemyComponent>()};
	if (IsValid(combatEnemyComp)) {combatEnemyComp->HandleResetAttack();}
}